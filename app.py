...
DEFAULT_TEMPLATE_FILE = os.path.join(BASE_DIR, "LabelTemplate_2026_Python.dymo")
    ...
    with _print_lock:
        ...
        for fx in fixtures:
            try:
                ...
                dymo_print.print_label_xml(filled, printer_name=printer)
                printed += 1
            except Exception as e:
                errors.append({"fid": fx.get("fid"), "error": str(e)})
        return {"printed": printed, "errors": errors, "total": len(fixtures)}
...
if __name__ == "__main__":
    ...
    print("DYMO label server starting...")
